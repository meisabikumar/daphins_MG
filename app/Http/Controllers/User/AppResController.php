...
class AppResController extends Controller
{
    ...
    public function Football_User_Teams(Request $request)
    {
        ...
        $teams=$request->post('team');
        $match_id=$request->post('match_id');
        $team_json=json_encode($teams);
        // return $team_json;
        $ret=DB::table('football_user_team')->insert(array("user_id"=>$user_id,"teams"=>$team_json,'match_id'=>$match_id));
    
        if($ret)
        {
            $res=DB::table('football_user_team')->where(array("user_id"=>$user_id,"match_id"=>$match_id))->get();
        ...
        $data=array(
            ...
            "player_credits"=>$value->player_credits,
        ...
        $ret=DB::table('football_player_team')->insert(array(
    ...
    public function Football_Player_Teams(Request $request)
    {
        // $user_id=$request->post('user_id');
        $teams=DB::table('football_user_team')->get();
        ...
        foreach ($teams as  $tm) {
            ...
            $team=$tm->teams;
            $obj=(json_decode($team));
            
        
            // // $team=$tm->teams;
            // $td=json_encode($team);
            // echo json_decode($td,true);
        // $team_data=json_encode($team);
        // $obj=json_decode($team_data,true);
        // echo $obj;
    ...
    // return $result;
        ...
        if($ret)
        {
            $r=DB::table('cron_records')->insert(array("status"=>"(Football_teamplayer table)teamplayer cron"));
            return response()->json(["status" => 1,"message" => "Success"]);
        }else{
            return response()->json(["status" => 2,"message" => "No Data Found or Some error occured"]);
        }
